...
| RNF-005 | Escalabilidade          | O sistema deve ser capaz de lidar com crescimento no número de usuários e registros sem perda de desempenho, suportando até **200 usuários concorrentes** com tempo de resposta médio menor que **2 segundos** e sendo capaz de armazenar e consultar até **1 milhão** de registros no banco sem impactar o desembenho. | MÉDIA      |
...
| RNF-009 | Portabilidade           | O sistema deve ser facilmente migrável para outras plataformas de cloud sem grandes alterações. Para isso vamos empacotar o sistema com Docker, usar um banco de dados externo e configurável, usar variáveis de ambiente, evitar o uso de serviços proprietários específicos e usar serviços que tem em qualquer cloud como Postgres, Redis e Kafka. | BAIXA      |
...
| ------- | ------------------- | ------------------------------------------------------------ |
| RST-001 | Prazo               | O sistema deve ser entregue dentro do período definido pelo cronograma. |
| RST-002 | Orçamento           | Somente serão utilizados serviços gratuitos de cloud, banco de dados e hospedagem. |
| RST-003 | Tecnologia          | O backend deve ser desenvolvido usando **Java 21**           |
| RST-004 | Banco de Dados      | O banco utilizado será o **PostgreSQL**                      |
| RST-005 | Infraestrutura      | O sistema deve ser empacotado com Docker.                    |
